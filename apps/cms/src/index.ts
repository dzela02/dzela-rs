...
const CONTENT_TYPES = [
  'api::project.project',
  'api::experience-entry.experience-entry',
  'api::post.post',
] as const;
...
export default {
  ...
  async bootstrap({ strapi }: { strapi: Core.Strapi }) {
    ...
    await createReadOnlyToken(strapi);
    await seedData(strapi);
  },
};

/**
 * Grant public (unauthenticated) read access to all three content types.
 * Safe to run on every boot — only inserts missing permissions.
 */
async function setPublicPermissions(strapi: Core.Strapi) {
  ...
  for (const contentType of CONTENT_TYPES) {
    for (const action of PUBLIC_ACTIONS) {
      const permissionUid = `${contentType}.${action}`;
      const existing = await strapi.db
        .query('plugin::users-permissions.permission')
        .findOne({ where: { action: permissionUid, role: publicRole.id } });

      if (!existing) {
        await strapi.db.query('plugin::users-permissions.permission').create({
          data: { action: permissionUid, role: publicRole.id },
        });
        strapi.log.info(`[bootstrap] Public permission granted: ${permissionUid}`);
      }
    }
  }
}
...
/**
 * Create a read-only full-access API token named "portfolio-web" if one does
 * not already exist, and print it to the console on first boot.
 */
async function createReadOnlyToken(strapi: Core.Strapi) {
  const tokenService = strapi.service('admin::api-token') as {
    list: () => Promise<Array<{ name: string; accessKey: string }>>;
  ...
  const exists = tokens.some((t) => t.name === 'portfolio-web');

  if (!exists) {
    const token = await tokenService.create({
      name: 'portfolio-web',
      description: 'Read-only token for apps/web',
      type: 'read-only',
      lifespan: null,
    });
    strapi.log.info(`\n\n🔑  API token created — add this to apps/web/.env.local:\n`);
    strapi.log.info(`    STRAPI_API_TOKEN=${token.accessKey}\n\n`);
  }
}

/**
 * Seed static data if the collections are empty.
 */
async function seedData(strapi: Core.Strapi) {
  const projectCount = await strapi.db.query('api::project.project').count({});
  if (projectCount > 0) return;

  strapi.log.info('[bootstrap] Seeding initial content...');
  ...
  for (const p of PROJECTS) {
    await strapi.db.query('api::project.project').create({ data: p });
  }
  strapi.log.info('[bootstrap] ✓ 6 projects seeded');

  const EXPERIENCE = [
    {
      order: 0, years: '2025 — now', role: 'Senior Software Engineer', company: 'Rivian',
      team: 'Visualization & Simulation apps', location: 'Remote',
      bullets: [
    ...
    {
      order: 1, years: '2023 — 2025', role: 'Lead JavaScript Developer', company: '167Pluto',
    ...
    {
      order: 2, years: '2022 — 2023', role: 'JavaScript Developer', company: 'Q Agency',
      location: 'Belgrade, Serbia',
      bullets: [
    ...
    {
      order: 3, years: '2021 — 2022', role: 'Frontend Developer', company: 'Neotech Solutions',
      location: 'Belgrade, Serbia',
      bullets: [
    ...
    {
      order: 4, years: '2021 — 2022', role: 'Freelance', company: 'UpWork', location: 'Remote',
      bullets: [
    ...
    {
      order: 5, years: '2020 — 2021', role: 'Web Developer', company: 'Liquidity Hunter FX',
    ...
    {
      order: 6, years: '2018 — 2020', role: 'Full-stack Developer', company: 'TRIBE Festival',
  ...
  for (const e of EXPERIENCE) {
    await strapi.db.query('api::experience-entry.experience-entry').create({ data: e });
  }
  strapi.log.info('[bootstrap] ✓ 7 experience entries seeded');

  const POSTS = [
    { year: '2026', date: '2026-03-02', title: 'Frame budgets are a contract, not a vibe', slug: 'frame-budgets-are-a-contract', read: '9 min', tags: ['visualization', 'webgl'], publishedAt: new Date() },
    { year: '2025', date: '2025-12-14', title: 'Streaming telemetry into a typed UI without leaking the wire format', slug: 'streaming-telemetry-typed-ui', read: '11 min', tags: ['typescript', 'visualization'], publishedAt: new Date() },
    { year: '2025', date: '2025-09-20', title: 'Branded types for money — a primer', slug: 'branded-types-for-money', read: '8 min', tags: ['typescript'], publishedAt: new Date() },
    { year: '2025', date: '2025-06-19', title: 'OpenAPI + Orval = no more integration meetings', slug: 'openapi-orval-no-integration-meetings', read: '10 min', tags: ['typescript', 'tooling'], publishedAt: new Date() },
    { year: '2025', date: '2025-04-06', title: 'Notes on a TurboRepo migration', slug: 'notes-on-a-turborepo-migration', read: '12 min', tags: ['monorepo', 'devex'], publishedAt: new Date() },
    { year: '2024', date: '2024-11-22', title: 'When discriminated unions replace microservices', slug: 'discriminated-unions-replace-microservices', read: '14 min', tags: ['architecture'], publishedAt: new Date() },
    { year: '2024', date: '2024-10-04', title: 'CI/CD as code — treating pipelines like the rest of the app', slug: 'cicd-as-code', read: '9 min', tags: ['devex'], publishedAt: new Date() },
    { year: '2024', date: '2024-09-09', title: 'satisfies, as const, and the art of saying nothing extra', slug: 'satisfies-as-const', read: '6 min', tags: ['typescript'], publishedAt: new Date() },
    { year: '2024', date: '2024-05-30', title: 'Mentorship, but make it asynchronous', slug: 'mentorship-asynchronous', read: '5 min', tags: ['team'], publishedAt: new Date() },
  ];

  for (const p of POSTS) {
    await strapi.db.query('api::post.post').create({ data: p });
  }
  strapi.log.info('[bootstrap] ✓ 9 posts seeded');
